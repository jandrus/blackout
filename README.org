...
- 󰦝 Encrypted Password/Note manager
- 󰦝 Encrypted TOTP manager
...
When directed to generate a passphrase, ~blackout~ will choose the given number of words randomly from this list and capitalize each chosen word with a probability of 0.5. Thus, by default, there are
LaTeX: $n^4096$ possibilities, where $n$ is the number words to be generated in the passphrase.
